import { Injectable, UnauthorizedException } from "@nestjs/common"; // imports necessary NestJS utilities
import { PassportStrategy } from "@nestjs/passport"; // Passport strategy to integrate with JWT
import { User, users } from "@workspace/repo"; // imports user model and repository for querying user data
import { ExtractJwt, Strategy } from "passport-jwt"; // imports utilities to handle JWT extraction and validation

import {
  ConfigKeys,
  ConfigService,
} from "../../modules/config/config.service.js"; // imports configuration keys and service for retrieving config
